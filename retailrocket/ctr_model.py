...
DATA_CONFIG = {
    'data_path': 'final_events_with_history.csv',
    'test_size': 0.2,
    'random_state': 42
}
...
OUTPUT_CONFIG = {
    ...
    'item_embedding_mapping': 'itemid_embedding_kv.pkl'  # 修改为直接保存映射
}
...
class MLPModel(nn.Module):
    ...
    def get_item_embeddings(self, item_indices):
        """获取itemid的embedding"""
        return self.embeddings['itemid'](item_indices)
...
class CTRDataProcessor:
    ...
    def load_and_preprocess_data(self):
        ...
        # 保存原始itemid列表
        self.unique_item_ids = data['itemid'].unique().tolist()
    ...
    def process_features(self, data):
        ...
        # 特征列表
        features = ['visitorid', 'itemid'] + [f'last_view{i}_itemid' for i in range(1,11)]
...
def train_ctr_model():
    ...
    print(f"Unique item IDs: {len(processor.unique_item_ids)}")
    ...
    # 保存itemid到embedding的映射
    model.eval()
    with torch.no_grad():
        # 创建itemid到embedding的映射字典
        item_embedding_dict = {}
        
        # 获取itemid编码器
        item_encoder = processor.label_encoders['itemid']
        
        # 处理所有原始itemid
        for item_id in processor.unique_item_ids:
            # 将原始itemid编码
            try:
                encoded_id = item_encoder.transform([str(item_id)])[0]
            except ValueError:
                # 如果itemid不在编码器中，跳过
                continue
            ...
            # 保存到字典
            item_embedding_dict[item_id] = embedding
        
        print(f"总共有{len(item_embedding_dict.keys())}个item")
        # 保存映射字典
        mapping_path = os.path.join(OUTPUT_CONFIG['model_dir'], OUTPUT_CONFIG['item_embedding_mapping'])
        with open(mapping_path, 'wb') as f:
            pickle.dump(item_embedding_dict, f)
        json_path = os.path.join(OUTPUT_CONFIG['model_dir'], 'item_embedding_kv.json')
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(item_embedding_dict, f, indent=2) 
        print(f"item ID to embedding mapping saved to {mapping_path}")
        print(f"Saved embeddings for {len(item_embedding_dict)} items")
