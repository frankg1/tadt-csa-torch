...
def generate_type_index(batch_size, seq_len, device):
    # 生成类型索引 [0,1,2,0,1,2,...] 长度为 3*seq_len
    type_idx = torch.arange(0, 3, device=device)  # [0, 1, 2]
    type_idx_repeated = type_idx.repeat(seq_len)  # [0,1,2,0,1,2,...], 长度 = 3*seq_len
    # 添加batch维度
    type_idx_repeated = type_idx_repeated.unsqueeze(0).repeat(batch_size, 1)  # [B, 3*seq_len]
    return type_idx_repeated


def generate_time_index(batch_size, seq_len, device, type_num=3):
    # 生成时间索引 [0,0,0,1,1,1,2,2,2,...] 长度为 3*seq_len
    time_idx = torch.arange(0, seq_len, device=device)  # [0, 1, 2, ..., seq_len-1]
    time_idx_repeated = time_idx.repeat_interleave(type_num)  # [0,0,0,1,1,1,2,2,2,...]
    # 添加batch维度
    time_idx_repeated = time_idx_repeated.unsqueeze(0).repeat(batch_size, 1)  # [B, 3*seq_len]
...
def evaluate_cumulative_reward(model, dataloader, device, item_emb_layer):
    """
    评估累计reward
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # 前向传播，一次性得到所有位置的预测
            (action_preds, return_preds, reward_preds, cur_emb, next_emb, cur_act_emb, 
             codebook_id, cur_codebook_id, nxt_codebook_id, weights, s_t_seq) = model(
                obs_seq, act_id_seq, rtg_seq, adv_seq,
                type_idx, time_idx, 
...
class TADT_CSA(nn.Module):
    ...
    def forward(self, obs_seq, act_id_seq, rtg_seq, adv_seq, type_idx, time_idx, tgt_mask=None, key_padding_mask=None):
        ...
        # codebook量化
        s_t, weights, codebook_id = self.codebook(obs_flat, adv_emb, B, L)        # [B*L, D]
...
def train_model(args):
    ...
    log_dir = f"./accum_logs/{args.dataset}_{args.obs_len}"
    ...
    # 获取数据集统计信息
    obs_len = args.obs_len  # 使用传入的obs长度
    obs_dim = 20  # obs维度 (假设所有数据集obs维度都是20)
    item_num = train_dataset.dataset.stats['num_actions'] # 从完整数据集获取item_num
    max_sign = max(train_dataset.dataset.stats['action_range']) + 1 # 从完整数据集获取max_sign
    print(f"Dataset stats: {train_dataset.dataset.stats}")
    print(f"Model config: obs_len={obs_len}, obs_dim={obs_dim}, item_num={item_num}, max_sign={max_sign}")
    print(f"Training with obs_length={args.obs_len}, max_seq_len={args.obs_len}")
    ...
    model = TADT_CSA(
        ...
        max_adv_value=5.0,
        max_sign=max_sign,
        max_seq_len=args.obs_len,  # 使用obs_len作为max_seq_len
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        print(f"\n[Train][Epoch {epoch}] Starting training...")
        # --- 新增：收集评估用的预测和标签 ---
        eval_predictions = []
        eval_labels = []
        for batch_idx, batch in enumerate(pbar):
        ...
        val_mean_reward, val_sum_reward = evaluate_cumulative_reward(model, val_dataloader, device, item_emb_layer)
