...
def calc_advs(rtg, gamma=0.9):
    adv_trend = torch.zeros_like(rtg)
    for t in range(1, L):
    ...
    for i_episode in range(num_episodes):
        ...
        while not done:
            ...
            if args.model_name == "IQL":
                ...
                # rtg_seq, adv_seq = compute_rtg_and_advantage_trend_from_rewards(torch.tensor(rtgs).unsqueeze(0).unsqueeze(2).to(device))
                action = agent.predict_single_step(torch.tensor(states,dtype=torch.float32).to(device).unsqueeze(0), action_input, torch.tensor(rtgs).unsqueeze(0).unsqueeze(2).to(device), torch.tensor(advs).unsqueeze(0).unsqueeze(2).to(device))
