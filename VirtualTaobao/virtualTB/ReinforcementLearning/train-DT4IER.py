        ...
        for epoch in range(num_epochs):
            ...
            for batch in dataloader:
                if args.model_name == "IQL":
                    ...
                    _, ce_loss, contra_loss, reward_loss = agent(obs_seq, act_id_seq.squeeze(), seq_lengths, rtg_seq, reward_gt, attention_mask)
    ...
    for i_episode in range(num_episodes):
        ...
        while not done:
            if args.model_name == "IQL":
                ...
                '''1、R0,s0,a0,R1,s1,f_a1'''
                if len(actions) == 0:
                    action_input = fake_action.unsqueeze(0)# batch_size,seq_len,action_dim
                else:
                    # print(torch.tensor(actions).to(device).shape)
                    # print(fake_action.unsqueeze(1).shape)
                    action_input = torch.cat([torch.tensor(actions).unsqueeze(0).to(device), fake_action.unsqueeze(1)], dim=1)
                    # batch_size,seq_len,2
                rtg_combined = torch.cat([torch.tensor(rtgs).unsqueeze(0).unsqueeze(2), torch.tensor(rtgs1).unsqueeze(0).unsqueeze(2)], dim=-1).to(device)
                # print(rtg_combined.shape)
                # for s in states:
                #     print(s.shape)
                # print(torch.tensor(states,dtype=torch.float32).to(device).unsqueeze(0).shape)
                action = agent.predict_single_step(torch.tensor(states,dtype=torch.float32).to(device).unsqueeze(0), action_input, rtg_combined)
                action_np = action.detach().cpu().squeeze(0).numpy()
