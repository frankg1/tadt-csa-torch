...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False):
        w = torch.matmul(q, k)
        # print("w shape after matmul:", w.shape)
        ...
        if head_mask is not None:
            w = w * head_mask
        # print("w shape:", w.shape)   # [B, H, T_q, T_k]
        # print("v shape:", v.shape)   # [B, H, T_k, D]
        outputs = [torch.matmul(w, v)]
        # print("matmul result:", torch.matmul(w, v).shape)
    ...
    def forward(
        ...
        a = attn_outputs[0]
        # print("a shape before merge_heads:", a.shape)
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        if config.add_cross_attention:
            self.crossattention = Attention(hidden_size, n_ctx, config, scale, is_cross_attention=False)
